    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (removePic.length > 0) {
            removePic.map((pre, index) => {
                formData.append(`remove[${index}]`, pre)
            })
        }
        ...
        filteredData.image = selectedFiles;

        // console.log(filteredData)

        const response = await updateHeroPic(pictures._id, filteredData)
        if (response) {
            onClose()
            refresh()
        }
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-11">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col">
                ...
                <div className='flex flex-wrap mt-2 gap-2'>
                    ...
                    {pictures.image?.map((pre, index) => {
                        if (!removePic.includes(pre)) {
                            return (
                                <div key={index} className='flex dark:text-white'>
                                    <input name='oldPrimaryImage' id={pre} type='radio' value={pre} className="peer hidden" />
